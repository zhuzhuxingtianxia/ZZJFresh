...
#import "UILabel+ZJTextAction.h"
#import <objc/runtime.h>
#import <CoreText/CoreText.h>
#import <Foundation/Foundation.h>

@interface ZJAttributeModel : NSObject
...
@implementation ZJAttributeModel

@end

@implementation UILabel (ZJTextAction)
...
- (id<ZJTapActionDelegate>)delegate
{
    return objc_getAssociatedObject(self, _cmd);
}
- (void)setDelegate:(id<ZJTapActionDelegate>)delegate
{
...
{
    [self zj_getRangesWithStrings:strings];
...
- (void)zj_addAttributeTapActionWithStrings:(NSArray <NSString *> *)strings
                                   delegate:(id <ZJTapActionDelegate> )delegate
{
    [self zj_getRangesWithStrings:strings];
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    [self zj_getTapFrameWithTouchPoint:point result:^(NSString *string, NSRange range, NSInteger index) {
        
        if (weakSelf.tapBlock) {
            weakSelf.tapBlock (string , range , index);
        }
        
        if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(zj_attributeTapReturnString:range:index:)]) {
            [weakSelf.delegate zj_attributeTapReturnString:string range:range index:index];
        }
        
        if (self.isTapEffect) {
            
            [self zj_saveEffectDicWithRange:range];
            
            [self zj_tapEffectWithStatus:YES];
        }
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (self.isTapAction) {
        if ([self zj_getTapFrameWithTouchPoint:point result:nil]) {
            return self;
        }
    }
...
- (BOOL)zj_getTapFrameWithTouchPoint:(CGPoint)point result:(void (^) (NSString *string , NSRange range , NSInteger index))resultBlock
{
    ...
    CGAffineTransform transform = [self zj_transformForCoreText];
    ...
    for (CFIndex i = 0; i < count; i++) {
        ...
        CGRect flippedRect = [self zj_getLineBounds:line point:linePoint];
        ...
        if (CGRectContainsPoint(rect, point)) {
            ...
            for (int j = 0; j < link_count; j++) {
                ...
                ZJAttributeModel *model = self.attributeStrings[j];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    if (self.isTapEffect) {
        ...
        [self performSelectorOnMainThread:@selector(zj_tapEffectWithStatus:) withObject:nil waitUntilDone:NO];
...
- (CGAffineTransform)zj_transformForCoreText
{
    return CGAffineTransformScale(CGAffineTransformMakeTranslation(0, self.bounds.size.height), 1.f, -1.f);
}

- (CGRect)zj_getLineBounds:(CTLineRef)line point:(CGPoint)point
{
    CGFloat ascent = 0.0f;
...
- (void)zj_tapEffectWithStatus:(BOOL)status
{
    if (self.isTapEffect) {
...
- (void)zj_saveEffectDicWithRange:(NSRange)range
{
...
- (void)zj_getRangesWithStrings:(NSArray <NSString *>  *)strings
{
    ...
    [strings enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (range.length != 0) {
            ...
            totalStr = [totalStr stringByReplacingCharactersInRange:range withString:[weakSelf zj_getStringWithRange:range]];
            
            ZJAttributeModel *model = [ZJAttributeModel new];
...
- (NSString *)zj_getStringWithRange:(NSRange)range
{
